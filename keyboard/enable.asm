...
; Alias for CODE segment -- so it can be written to in protect mode.
...
; Old saved interrupt trap, defined in TRAP.ASM

    EXTRN bios_proc:DWORD

; This contains a copy of the NMI interrupt vector.
...
; reenabling after a fullscreen (good) app.
	...
	mov	ax, CSAlias
	cmp	ax, 0				; did we do this already?
	jnz	GotCSAlias			; if not,
	cCall	AllocCStoDSAlias,<cs>		; make alias for CODE
    public SaveCSAlias				; for debug...
    SaveCSAlias:
	mov	CSAlias, ax			; save for later
public GotCSAlias
GotCSAlias:

; Save away current keyboard interrupt vector value (in protected
; CODE segment now)

	push	ax				; save CS alias
	mov	ax,3500h or vector
	int	21h				; vector is in ES:BX
	pop	ax				; get CS alias
	push	ds
	mov	ds,ax				; DS = CODE alias
	mov	ax,bx
	mov	bx,codeOFFSET bios_proc
	mov	WORD PTR ds:[bx],ax		; save offset of vector
	mov	WORD PTR ds:[bx+2],es		; save segment of vector
	pop	ds
