...
; These pointers MUST be in  the CODE segment!

public bios_proc
bios_proc   DD	0			; Saved address of BIOS kbd.
					; interrupt handler (09H)
...
; Control and Alt are BOTH down now.  (but it's not AltGr, since that
; doesn't set the BIOS flags!)

ifdef	ENVOY

; If this a cCCPDel on the envoy keyboard, we need to reset ourselves.
; Note: Since Envoy is operating in RAW mode, the bios won't do it for
; us.
;
	cmp	al, cCCPDel		; cCCPDel can only be generated on
					; [ target label changed 16apr90: ]
	jne	kbi1			; the Envoy keyboard in RAW mode.
	call	KbdRst			; (see similar call below)
	mov	word ptr es:[fReset], 1234h	;  Soft reset the system
	jmp	ds:[ResetVector]
CheckDel:

endif ; ENVOY

; Again, control and Alt are BOTH down now. We're going to BIOS
; if its cDelete..
...
kbToBios:
	...
	pop	ds
	pop	ax
	jmp	cs:[bios_proc]		; let ROM handle CTRL-ALT del,
					;  break, print (or anything else)

; Check for OS/2 hotkey, then read keyboard status and reset the keyboard
...
; Alt key up or down?  If so, pass it through the OS/2 driver
	...
	mov	ds:[fIgnoreKey],0   	; initial state
	cmp	ds:[fIgnoreEsc],0   	; ignoring Esc up?
	jz	kbi14			; nope..
	mov	ds:[fIgnoreEsc],0   	; clear the flag
	cmp	al,cEsc+80h		; up of the Esc key?
	jnz	kbi14			; no - Esc was lost - don't ignore key
	inc	ds:[fIgnoreKey]	 	; set flag to ignore key
...
; we ignore this key (but correctly set our key state...)
	...
	inc	ds:[fIgnoreKey]		; set the flag to ignore
	inc	ds:[fIgnoreEsc]	 	; set the "ignore Esc key" flag
...
; next 2 lines added 09dec87
	cmp	ds:[fIgnoreEsc],0   	; About to switch ?
...
kbi11:
	; fix the stack .. 
	pop	es			; restore ES & DS from the stack
	pop	ds			; 
	push	ds			; DS & ES & AX still on stack...
	push	es
...
; ALT SCR-PRT for screen grab.
	...
	jne	notsys
	cmp	fOS2Box,0		; in OS/2 compat. box?
	jnz	notsys			; .. yep, we're not debugging Windows!
					; (CtlAltSysReq confilicts with OS/2
					; usage).
...
;;	je	igkey			;  if so, ignore this.
	...
	pop	ax
	jnz	@F
	jmp	cs:[nmi_vector]		; go to SYMDEB!!
