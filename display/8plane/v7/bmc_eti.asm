...
;
	...
	mov	mono_shift_align_count,0; flush align for target color maps
...
eti_24_not_mono:
	...
	mov	bx, DIMapSegOFFSET copy_e24i8_partial
...
;
	...
	mov	int_move_count, ax
;
	mov	ax,bx			; get the orginal amount
	and	ax,0011b		; if already aligned, skip the calc
	jz	eti_24_isaligned
        sub     ax,4
	neg	ax
;
...
; copy_e24i8								     ;
;									     ;
;   copy external 24 bit RGB to	internal 4 plane			     ;
;									     ;
;   The	source is copied to all	four planes of the destination,	with	     ;
...
;
        stosb                           ; Save C0
	dec	bl
	jne	ce24i8_next_pixel
;
	loop	ce24i8_next_byte	; complete all the target bytes
	pop	es
        ret
...
; copy_e24i8_partial							     ;
;									     ;
;   copy external 24 bit RGB to	internal 4 plane, partial byte		     ;
;									     ;
;   The	source is copied to all	four planes of the destination,	with	     ;
;   color conversion via the actual RGB	triplet.			     ;
;									     ;
...
;
        stosb
	dec	cl
	jne	ce24i8p_next_pixel	; process all left over	pels
;
ce24i8p_save_byte:
	pop	es
        ret
