...
%OUT take this out!!!
	int 1
	lds	si,lpColorInfo	; to see what is there..
ENDIF
	cld
...
; get a	long pointer to	the start of the destination bitmap
	...
	les	di,[si].bmBits
	mov	off_lp_bits_dev, di
	mov	seg_lp_bits_dev, es

        les     bx     ,lp_info_ext
	cmp	BYTE PTR es:[bx].biCompression,BI_RLE8 ; RLE_FORMAT_8
	je	encoded
	cmp	BYTE PTR es:[bx].biCompression,BI_RLE4 ; RLE_FORMAT_4
	jne	not_encoded
...
not_encoded:
	mov	ax, WORD PTR [lp_bits_ext]
	or	ax, WORD PTR [lp_bits_ext + 2]	; test for zero
	jz	error_detected			; NULL pointer ---- invalid
