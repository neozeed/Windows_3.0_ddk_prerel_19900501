...
;
	...
	call	set_bank_select		; do the video h/w bank select
;
@@:
;
rtIF	BigFontFlags
...
;
	mov	ss_draw,offset color_opaque_output
	test	accel,IS_OPAQUE
	jnz	@F
	mov	ss_draw,offset color_transp_output
;
@@:
;
...
@@:
	...
	mov	bx,ss_phases		; lenght of map less char width
	sub	bx,cx
	mov	ss_next_scan,bx		; holds offset to next scan line
	mov	bx,di
	add	bx,cx
...
; load the output routine
;
	mov	ss_draw,offset pcolor_opaque_output
	test	accel,IS_OPAQUE
	jnz	@F
	mov	ss_draw,offset pcolor_transp_output
;
@@:
;
; calculate the number of pixels (bytes) to be modified
