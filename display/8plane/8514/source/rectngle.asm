...
;	 dec	 bx
	jle	RTError 		;if zero or less, get out
	mov	si,Y2
	mov	cx, Y1
...
;	 dec	 si
	...
	clc				;clear carry - normal operation
	ret
RTError:
	stc				;set carry - error exit
	ret
DrawInterior	endp
...
;RT1HighOkay:
	...
	or	ax, ax
	js	RT2Negative
	cmp	ax, bx			;if ending X is on or below the lower
	jbe	RT2Negative		;limit for X, then exit
	cmp	ax, cx
	jna	RT2HighOkay
	mov	ax, cx
	or	di, CLIPPED_HIGH
