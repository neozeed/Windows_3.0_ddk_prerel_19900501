...
cblt_6000:
	...
	mov	bx, di			;we might need it a little later
	...
	jz	cblt_6100		;no source? then things are okay as is
	cmp	gl_first_fetch, 0	;only 1 byte ti fetch initially?
	jne	cblt_6100		;no.  need to fetch a last byte as well
	cmp	gl_phase_h, 0		;is horz. phase 0?
	je	cblt_6100		;yes, leave the last byte fetch in
	mov	bptr es:[bx], I_NOP	;get rid of lodsb instruction
