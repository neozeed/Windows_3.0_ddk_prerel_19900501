...
externW OldFontSize
externW OldFontHeight
externW BigOldFontSize
externW BigOldFontHeight
...
BSBCheckCache:
	...
	mov	si,[si+39]		;DS:SI points at facename text (add?)
	mov	cx, wFaceNameLength	;CX: facename length
	mov	di, pwOldFaceName	;ES:DI-->previous facename
	mov	ax,PointSize		;get the point size
	mov	bx, pwOldFontSize
	cmp	ax, es:[bx]		;is it different from the old one?
	jne	BSBDifferentFont	;yes
	mov	ax, FontHeight		;now check if the actual font height
	mov	bx, pwOldFontHeight	;in pixels is the same as in the
	cmp	ax, es:[bx]		;current font.	same height?
	jne	BSBDifferentFont	;no. Load new font, update cache params
...
repe	cmpsb				;compare the facenames
	...
	pop	si
	jcxz	BSBLoadNewFont		;same font. get new characters if needed

BSBDifferentFont:
	dec	bNewFont		;signal: need to load a new font

BSBUpdateFacename:
	mov	cx, wFaceNameLength	;we need to replace the facename
rep	movsb				;save the new facename
	mov	bx, pwOldFontSize
	mov	ax, PointSize		;save the new font size
	mov	es:[bx], ax
	mov	ax, FontHeight		;save new font height in pixels
	mov	bx, pwOldFontHeight
	mov	es:[bx], ax
