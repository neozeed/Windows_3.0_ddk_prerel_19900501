...
;First, get and save the length of the ROP and the operands contained within:
        ...
        mov     seg_Rop,dx              ;and pretend we're not above 80h (so
                                        ;we don't NOT later)
        jmp     short BBGetOperands     ;continue
...
;DX still contains the ROP code.
        ...
        jz      BBNoAction              ;if there are no operands, get out
...
BBSaveRopCount:
        ...
        jmp     short BBGetOperands     ;and continue
...
;There's not enough space.  See if there's a saved screen that we can trash:

	test	SaveInUse, SAVEBITMAPUSED	;is SaveScreenBitmap using
                                                ;off-screen memory?
	jz	GFRExit 			;nope, no adjustment needed

;There's a saved area that we can trash.  Set the ShadowMemoryTrashed flag
;and reclaim the area used by SaveScreenBitmap:

	mov	ShadowMemoryTrashed, 0	;free save screen or font
	mov	ax,SaveSpaceyOrg	;get the origin of
					;SaveScreenBitmap's allocation
	mov	bx,FreeSpaceyOrg	;get the current starting Y of
					;free space
	mov	FreeSpaceyOrg,ax	;set the start of free space to
					;the start of SaveScreenBitmap's
					;allocation
	sub	bx,ax			;get the nbr of lines reclaimed
	add	LocFreeSpaceyExt,bx	;and update nbr of lines available
	add	FreeSpaceyExt, bx
