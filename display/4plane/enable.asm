...
;----------------------------------------------------------------------------;
	...
	jz	@f			;no
	jmp	inquire_gdi_info	;  Yes, return GDIinfo
@@:
...
; this completes the initializations
...
;----------------------------------------------------------------------------;
	...
	mov	cx,__WinFlags
	and	cx,WINP + WIN386	;mask the required bits
	cmp	cx,WINP + WIN386	;are we 386 and in protected mode ?
	jz	driver_info_ok		;yes
