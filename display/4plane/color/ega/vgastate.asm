...
; read the read map register. The plane index location has the index value
; of each plane set in each of the planes, so reading it is still a better 
; way of geting the value of the read map register.
...
; program the mode register to be write mode 0, read mode 0
	...
	mov	ax,(M_DATA_READ + M_PROC_WRITE) shl 8 + GRAF_MODE
	out16	dx,ax
