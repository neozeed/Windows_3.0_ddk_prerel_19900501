...
\*---------------------------------------------------------------------------*/
HBITMAP FAR PASCAL CreateBrushBitmap(hDC,nWidth,nHeight,hBrush)
     HDC            hDC;
...
|   HBITMAP - Handle to a Device Dependant Bitmap.                            |
\*---------------------------------------------------------------------------*/
HBITMAP FAR PASCAL CreatePixelBitmap(hDC,x,y)
     HDC      hDC;
     register short x;
     register short y;
{
...
|   HBITMAP - Handle to a Device Dependant Bitmap.                            |
\*---------------------------------------------------------------------------*/
HBITMAP FAR PASCAL CreateRGBBitmap(hDC,nWidth,nHeight,rgb)
     HDC      hDC;
     short    nWidth;
     short    nHeight;
     COLORREF rgb;
...
\*---------------------------------------------------------------------------*/
BOOL FAR PASCAL OutputDDBToDevice(hDC,xStart,yStart,hBitmap,dwROP)
     HDC              hDC;
...
\*---------------------------------------------------------------------------*/
HANDLE FAR PASCAL ConvertDDBToDIB(hDC,hBitmap,wDIBType,dwCompress)
     HDC     hDC;
     HBITMAP hBitmap;
     WORD    wDIBType;
     DWORD   dwCompress;
{
     ...
     {
          case 1:
               nTblSize = (wDIBType == DIB_OS2 ? 2*sizeof(RGBTRIPLE) : 2*sizeof(RGBQUAD));
               break;
          case 4:
               nTblSize = (wDIBType == DIB_OS2 ? 16*sizeof(RGBTRIPLE) : 16*sizeof(RGBQUAD));
               break;
          case 8:
               nTblSize = (wDIBType == DIB_OS2 ? 256*sizeof(RGBTRIPLE) : 256*sizeof(RGBQUAD));
               break;
          default:
               nTblSize = 0;
               break;
     ...
     switch(wDIBType)
     {
          case DIB_OS2:
               ...
               if(!(hdib = GlobalAlloc(GHND,(DWORD)(sizeof(BITMAPCOREHEADER)+nTblSize))))
                    return(NULL);
               if(!(lpbc = (LPVOID)GlobalLock(hdib)))
               {
                    GlobalFree(hdib);
                    return(NULL);
               }
               *lpbc = bmc;
               ...
               if(!GetDIBits(hDC,hBitmap,0,(WORD)bmc.bcHeight,(LPSTR)lpbc+lpbc->bcSize+nTblSize,
          ...
          default:
               ...
               if(!(hdib = GlobalAlloc(GHND,(DWORD)(sizeof(BITMAPINFOHEADER)+nTblSize))))
                    return(NULL);
               if(!(lpbi = (LPVOID)GlobalLock(hdib)))
               {
                    GlobalFree(hdib);
                    return(NULL);
               }
               *lpbi = bmi;
               ...
               if(!GetDIBits(hDC,hBitmap,0,(WORD)bmi.biHeight,NULL,(LPBITMAPINFO)lpbi,DIB_RGB_COLORS))
               {
                    GlobalUnlock(hdib);
                    GlobalFree(hdib);
                    return(NULL);
               ...
               if(!GetDIBits(hDC,hBitmap,0,(WORD)bmi.biHeight,(LPSTR)lpbi+lpbi->biSize+nTblSize,
...
\*---------------------------------------------------------------------------*/
BOOL FAR PASCAL OutputDIBToDevice(hDC,x,y,hdib,dwRop)
     HDC            hDC;
     register short x;
     register short y;
     HANDLE         hdib;
     DWORD          dwRop;
{
     ...
     if(lpdib = (LPVOID)GlobalLock(hdib))
     {
          ...
          SetDIBitsToDevice(hDC,x,y,(WORD)lpdib->biWidth,(WORD)lpdib->biHeight,
               0,0,0,(WORD)lpdib->biHeight,(LPSTR)lpdib+lpdib->biSize+nTblSize,
...
int FAR PASCAL GetColorTableSize(lpdib)
     LPBITMAPINFOHEADER lpdib;
{
     LPBITMAPCOREHEADER lpbc;
     LPBITMAPINFOHEADER lpbi;
...
BOOL FAR PASCAL CompareBitmaps(hDst,hSrc)
     HBITMAP hDst;
     HBITMAP hSrc;
{
     BITMAP         bm;
