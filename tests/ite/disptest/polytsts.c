...
BOOL DisplayPolygons(hWnd,hDC,lpDevCaps,hDevBrushes,hDevPens,hDevFonts,lpNextTest)
     HWND       hWnd;
     HDC        hDC;
     LPDEVINFO  lpDevCaps;
     ...
     LPSTR      lpNextTest;
{
     ...
     int         x,y,dx,dy,nOldBack,nOldPoly,nOldROP,nObj;
     RECT        rRect;
     LOCALHANDLE hBuffer;
     ...
     static PRINTCAPS szFill[] = {ALTERNATE     , "Alternate Fill Mode",
                                  WINDING       , "Winding Fill Mode"};
     ...
     if((nIdy >= GetObjectCount(tlTest.gtTest.hBrushes)) &&
        (nIdx >= GetObjectCount(tlTest.gtTest.hPens)))
     {
          nIdx = 0; nIdy = 0;
          (*lpNextTest)++;
          return(TRUE);
     ...
     for(nRow=0; nRow < 4; nRow++)
     {
          for(nCol=0; nCol < 4; nCol++)
          {
              ...
              \*--------------------------------*/
              nOldPoly = SetPolyFillMode(hDC,szFill[nIdz].nIndex);
              ...
              TstDrawObject(hDC,(nCol*(nObjW+nHorzS))+nHorzS,
                   ...
                   nObjW,nObjH-nTxtH,OBJ_POLYGON);
              SetROP2(hDC,nOldROP);
              SetPolyFillMode(hDC,nOldPoly);
     ...
     if(hBuffer = LocalAlloc(LHND,128))
     {
          if(lpBuffer = LocalLock(hBuffer))
          {
               ...
               wsprintf(lpBuffer,"Fill Mode - %s   Back Mode - %s          ",
                    (LPSTR)szFill[nIdz].szType,(LPSTR)szBack[nIdw].szType);
