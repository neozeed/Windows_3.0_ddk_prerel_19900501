...
emode_for_1st:
	...
	btr	[esi.DMA_flags], ecx	; clear word cnt bit
	cmp	al, DMA_EM_16bit_wc
	jne	short not_word_cnt_chn
	bts	[esi.DMA_flags], ecx	; set word cnt bit
not_word_cnt_chn:
	ret
...
VDMAD_GetHiPage:
	...
	movzx	eax, word ptr [ecx*4][esi.DMA_adrs+3]	 ; al = A24-A31
...
VDMAD_SetHiCount:
	...
	mov	byte ptr [ecx*4][esi.DMA_counts+2], al	; set high count byte
	xor	edx, edx		; check mask bit
	CallRet VDMAD_NotifyOwner

VDMAD_GetHiCount:
	call	channel_from_hicount_port
	mov	al, byte ptr [ecx*4][esi.DMA_counts+2]	; set high count byte
