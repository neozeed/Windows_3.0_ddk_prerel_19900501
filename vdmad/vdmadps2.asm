...
;   ENTRY:	    EAX = channel #
;		    EDI - channel MOD 4
;		    ESI -> DMA_Controller_State in VM's CB
;
;   EXIT:
;
;   USES:	    EAX, flags
...
;
;==============================================================================
BeginProc VDMAD_PS2_Init_VM_CB
...
ENDIF
	...
	mov	eax, edi
	call	VDMAD_Read_PS2_Mode	    ; read mode in first 4 channels

	mov	eax, edi
	add	eax, 4
	push	esi
	add	esi, SIZE DMA_Controller_State
	call	VDMAD_Read_PS2_Mode	    ; read mode in 2nd 4 channels
	pop	esi
...
; get I/O from VM's control block (virtual state)

	mov	ebx, [esp.Pushad_EBX]
	add	ebx, [VDMAD_CB_Offset]
	cmp	al, 4			;Q: channel of 2nd controller?
	jb	short spps_ctrl_1	;   N:
	sub	al, 4			;   Y:
...
spps_ctrl_1:
	...
	out	DMA_XFN, al		; out command byte
	IO_Delay
	movzx	eax, [edi*2][ebx.DMA_IO_ports]
...
;
; Get next available DMA data byte from the VM's control block
;
PS2_Get_DMA_Byte:
...
;
; Put the data byte into Base, Page or Count field in DMA controller data in
; VM's control block based upon the setup from the last executed CMD written
...
;
PS2_New_Mode:
	call	PS2_Get_Ctl_Data_Ptr
	mov	[edx][edi.DMA_Ext_Modes], al

;
; modify simulated 8237 mode with verify, read, write operation type
;
	mov	ah, [edx][edi.DMA_modes]
...
PS2_New_IO_Adr:
	call	PS2_Get_Ctl_Data_Ptr
	lea	edi, [edx*2][edi.DMA_IO_ports]
...
;******************************************************************************
;
;   PS2_Get_Ctl_Data_Ptr
;
;   DESCRIPTION:    Get pointer to virtual 8237 DMA controller data in the
;		    VM's control block
;
;   ENTRY:	    EBX is VM handle
;		    ESI points to PS/2 DMA data in VM's CB
;
;   EXIT:	    EDI points to DMA controller data in VM's CB
;		    EDX is channel # within controller (0-3)
;
;   USES:
;
;==============================================================================
BeginProc PS2_Get_Ctl_Data_Ptr

	mov	edi, ebx
	add	edi, [VDMAD_CB_Offset]
	movzx	edx, [esi.DMA_PS2_cmd]
	and	dl, Channel_Mask
	cmp	dl, 4			    ;Q: 2nd controller
	jb	short ef_1st_ctl	    ;	N:
	and	dl, DMA_chan_sel	    ;	Y: make it controller relative
	add	edi, SIZE DMA_Controller_State	; point to 2nd controller data
ef_1st_ctl:
	ret

EndProc PS2_Get_Ctl_Data_Ptr
...
;		    up state in VM's control block for subsequent writes to
;		    the extended function port
;
;   ENTRY:	    EAX = channel #
;		    EBX = VM handle
;		    ESI points to controller data in the VM's control block
;		    EDX = command byte
;
;   EXIT:
;
...
BeginProc PS2_DMA_Set_IO_Adr
	...
	and	al, 3
	movzx	eax, [eax*2][esi.DMA_IO_ports]	; eax = current port addr
	mov	cl, 2
	mov	ch, TRUE
...
PS_DMA_GetSet_Mem_Adr:
	and	al, 3
	mov	eax, [eax*4][esi.DMA_adrs]  ; eax = current addr
	mov	cl, 3
	CallRet PS2_Queue_DMA_Data
...
PS_DMA_GetSet_Count:
	and	al, 3
	mov	eax, [eax*4][esi.DMA_counts]	; eax = current count
	mov	cl, 2
	CallRet PS2_Queue_DMA_Data
...
BeginProc PS2_DMA_Set_Mode
	...
	and	al, 3
	movzx	eax, [eax][esi.DMA_Ext_Modes] ; eax = current mode
	mov	cl, 1
	mov	ch, TRUE
	CallRet PS2_Queue_DMA_Data
...
BeginProc PS2_DMA_Set_Chn_Mask
	...
	mov	ecx, eax
	and	cl, DMA_chan_sel
	bts	dword ptr [esi.DMA_mask], ecx
	mov	edx, 1
...
BeginProc PS2_DMA_Reset_Chn_Mask
	...
	mov	ecx, eax
	and	cl, DMA_chan_sel
	btr	dword ptr [esi.DMA_mask], ecx
	mov	edx, 1
	CallRet VDMAD_NotifyOwner
...
BeginProc PS2_DMA_Master_Clear
	...
	add	esi, [VDMAD_CB_Offset]	    ; esi points to DMA data
	xor	eax, eax
	mov	[esi.ctl1.DMA_status], al
	mov	[esi.ctl2.DMA_status], al
	mov	[esi.ctl1.DMA_request], al
	mov	[esi.ctl2.DMA_request], al
	mov	[esi.ctl1.DMA_flipflop], al
	mov	[esi.ctl2.DMA_flipflop], al
	mov	[esi.ctl1.DMA_modes], DMA_single_mode
	mov	[esi.ctl2.DMA_modes], DMA_single_mode
	mov	[esi.ctl1.DMA_Ext_Modes], 0
	mov	[esi.ctl2.DMA_Ext_Modes], _16_bit_xfer
...
;   PS2_Queue_DMA_Data
;
;   DESCRIPTION:    Queue DMA data into the PS/2 specific data in the VM's
;		    control block.  The data is then read from on subsequent
