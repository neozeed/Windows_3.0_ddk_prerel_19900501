...
HANDLE FAR PASCAL
AddFonts(
   hDB, hMd, iddstLB, hdstSFlb, idsrcLB, hsrcSFlb, lpModNm, lpPortNm, lpCount)
    HWND hDB;
    HANDLE hMd;
    WORD iddstLB;
    HANDLE hdstSFlb;
    ...
    *lpCount = 0;
	...
	if (GetTargDir(hDB,hMd,lpBuf->path,sizeof(lpBuf->path),lpAppNm))
	    {
		...
		for (sflb=&lpsrcSFlb->sflb[ind=0];
			...
			if (needPFM)
			    ...
			    /*  Build the PFM file.
                             ...
                             */
			    if (!existDL ||
				!buildPFM(hDB,hMd,lpBuf,sflb->indSFfile))
				{
