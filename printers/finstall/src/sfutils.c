...
//  History (last first)
//
//  11 jan 90	peterbe	Add numsoftsel to keep track of whether to enable
//			perm/temp buttons.  Also, ALT+SHIFT+CTRL clears
//			all highlights in the listbox now.
...
#define DBGsoftsel(msg)   DBMSG(msg)
...
HANDLE FAR PASCAL FillListBox(hDB, hMd, idLB, lpModNm, lpPortNm)
	...
	case CLASS_DESKJET_PLUS:
		nTitle = (idLB == SF_LB_LEFT) ?
				SF_LDESKJETPLUS : SF_RDESKJETPLUS;
		break;
	case CLASS_LASERJET:
	default:
		nTitle = (idLB == SF_LB_LEFT) ? SF_LDRIVERNM : SF_RDRIVERNM;
		break;
	}
...
WORD FAR PASCAL UpdateStatusLine(hDB, idLB, dlgState, hSFlb, selectAll)
	...
	{
	//  Highlight everything in the listbox if SHIFT & ALT were down
	//  when this was called; but if the ALT key is also down, then
	//  we UNhighlight everything (and clear the selectAll flag).
	//  Hope you like assignments imbedded in expressions!
	if (selectAll)
	    {
	    SendDlgItemMessage(hDB, idLB, LB_SETSEL, 
	    	(selectAll = (GetKeyState(VK_MENU) >= 0)), (long)(-1));
	    }
	...
	/*  Traverse the list modifying our record of which items
         ...
         */
	for (numsel=numsoftsel=ind=0, selected=sfind=-1, sflb=&lpSFlb->sflb[0];
		...
		// Count number of soft fonts selected.
		if ((sflb->state & SFLB_CART) == 0)
		    numsoftsel++;
		}
	    else
		{
	...
	// debug -- display count of soft fonts selected
	DBGsoftsel(("UpdateStatusLine(): numsoftsel=%d\n", numsoftsel));
	...
	if ((sfind > -1) && (sflb = &lpSFlb->sflb[sfind]) &&
	    ...
	    {
	    // Enable perm/temp buttons if the current selection is a
	    // soft font AND the only one selected.
	    BOOL fAllowTempPerm = ((dlgState&SFDLG_CARTRIDGE)==0) &&
				    (numsoftsel == 1);
...
void FAR PASCAL UpdateControls(hDB, dlgState)
    ...
    WORD dlgState;
    {
    BOOL fAllowSFOps = (dlgState&SFDLG_CARTRIDGE)==0;
