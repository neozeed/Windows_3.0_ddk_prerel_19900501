...
//
// 14 mar 90	clarkc		Overhaul.  Fixed lockSFdirEntry() by checking
//                              if lpSFentry set.  If not, check booleans to
//                              see what cleanup is necessary.  Changed
//                                  !(sf->state & SF_UNIQ & SF_FDEL)      to
//                                  !(sf->state & (SF_UNIQ | SF_FDEL))
//                              inside addSFdirEntry().  Last parameter of
//                              replaceSFdirEntry() changed to WORD.  Casting
//                              of unsigned shorts to signed longs everywhere,
//                              with the change of sf->offset = r - lpDir; to
//                              sf->offset = (long) r - (long) lpDir; within
//                              compressSFdir() being the most significant.
...
#define DBGaddsf(msg)	    /*nul*/
#define DBGcompress(msg)    /*nul*/
#define dumpTstate(TOKENSTATE) /*null*/
#define dumpSFstate(s)      /*null*/
...
LOCAL void unlockSFdir(LPSFDIR);
...
LOCAL int replaceSFdirEntry(LPSFDIR, int, LPSTR, WORD, WORD);
...
LPSTR FAR PASCAL lockSFdirEntry(lpSFdir, sfind)
    ...
    LPSFDIRENTRY sf;
    LPSTR lpSFentry = 0L;
    BOOL bLockedOnEntry = (lpSFdir || gLPSFdir);
    BOOL bgLPSFdir      = !(gLPSFdir == 0L);
	...
	if ((sfind > -1) && (sfind < lpSFdir->len))
        ...
        if (!lpSFentry)
	    {
	        --gCountLocks;
                if (!bLockedOnEntry)
	            {
	                unlockSFdir(gLPSFdir);
	            }
                if (!bgLPSFdir || !bLockedOnEntry)
	            {
	                gLPSFdir = 0L;
	            }
	    }
...
LOCAL LPSFDIR splitFreeSpace(lpSFdir, freeInd, lpNullInd, size)
    ...
    *lpNullInd = -1;
    ...
    lpNull->offset = lpFree->offset + (long) size;
    lpFree->size = size;
...
LOCAL WORD compressSFdir(lpSFdir, lpFreeOffset)
	...
	if (size)
		...
		if (!sizeleft)
		    ...
		    sizeleft = sf->size;
		    sf->offset = (long) r - (long) lpDir;
		    }
		*r = *q;
		}
	    freeSize += size;
	    freeOffset -= (long) size;
	    end -= size;
	    size = 0;
	    }
	else
...
void FAR PASCAL dumpSFbuf(lpSFdir)
	...
	p < end; p += sf->size, offset += (long) sf->size)
	{
	ind = *((int FAR *)p);
	sf = &lpSFdir->sf[ind];
