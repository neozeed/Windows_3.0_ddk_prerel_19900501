...
LOCAL char GetFontDef(LPSFDIRTKSTATE);
LOCAL char GetCartridgeName(LPSFDIRTKSTATE);
LOCAL char GetScrnFont(LPSFDIRTKSTATE);
LOCAL char GetPackage(LPSFDIRTKSTATE);
LOCAL char EndPackage(LPSFDIRTKSTATE);
LOCAL char GetFamily(LPSFDIRTKSTATE);
LOCAL char EndFamily(LPSFDIRTKSTATE);
LOCAL char GetCartridge(LPSFDIRTKSTATE);
LOCAL char GetLogDrive(LPSFDIRTKSTATE);
LOCAL char DoNuthing(LPSFDIRTKSTATE);

LOCAL char GetString(int, LPSTR, int, char, char);
LOCAL char GetDriveID(int, LPSTR, int, char);
LOCAL char GetFileName(int, LPSTR, int, char);
LOCAL char GetToken(int, LPSTR, int, char);
...
LOCAL char DumpLine(int, LPSTR, int, char);
LOCAL BOOL PushTstate(LPSFDIRTKSTATE, LPSTR);
LOCAL BOOL PopTstate(LPSFDIRTKSTATE, char);
...
typedef char (*GETPROC)();  /* ptr to a "C" Get<token> function. */

typedef struct {
    char name[TK_NAMELEN];
    GETPROC proc;
    } TOKENREC;

typedef struct {
    char buf[TMPBUF_BUFSZ];		/* MUST be first */
    SFDIRTKSTATE tstate;
    char errbuf[TMPBUF_STRSZ];		/* Note: WriteErr() depends upon */
    char linenum[TMPBUF_STRSZ];		/* these strings following the   */
    char aroundch[TMPBUF_STRSZ];	/* tstate struct.                */
    char aroundeol[TMPBUF_STRSZ];
...
/*  GetFontDef
 */
LOCAL char GetFontDef(lpTstate)
...
/*  GetScrnFont
 */
LOCAL char GetScrnFont(lpTstate)
    LPSFDIRTKSTATE lpTstate;
    {
...
/*  GetPackage
 */
LOCAL char GetPackage(lpTstate)
    LPSFDIRTKSTATE lpTstate;
    {
    char ch = lpTstate->ch;
...
/*  EndPackage
 */
LOCAL char EndPackage(lpTstate)
    LPSFDIRTKSTATE lpTstate;
    {
    char ch = lpTstate->ch;
 ...
 */
LOCAL char GetCartridgeName(LPSFDIRTKSTATE lpTstate)
    {
    LPSFDIRFILE lpSFfile = (LPSFDIRFILE)lpTstate->lpBuf;
    LPSTR lpBuf = (LPSTR)lpSFfile->s;
    LPSTR j, k;
 ...
 */
LOCAL char GetCartridge(LPSFDIRTKSTATE lpTstate)
    {
    LPSTR lpBuf = lpTstate->lpBuf;
    int bufsz = lpTstate->bufsz;
    char ch = lpTstate->ch;
 ...
 *  word "DRIVE."
 */
LOCAL char GetLogDrive(lpTstate)
    LPSFDIRTKSTATE lpTstate;
    {
 ...
 */
LOCAL char GetString(hFile, lpBuf, size, ch, endswith)
    int hFile;
    LPSTR lpBuf;
    int size;
    char ch;
 ...
 */
LOCAL char GetDriveID(hFile, lpBuf, size, ch)
    int hFile;
    LPSTR lpBuf;
    int size;
    char ch;
 ...
 */
LOCAL char GetFileName(hFile, lpBuf, size, ch)
    int hFile;
    LPSTR lpBuf;
    int size;
    char ch;
 ...
 *  processed.
 */
LOCAL char GetToken(hsrcfile, lpBuf, size, prevch)
    int hsrcfile;
    LPSTR lpBuf;
    int size;
    char prevch;
 ...
 */
LOCAL char DumpLine(hFile, lpBuf, size, ch)
    int hFile;
    LPSTR lpBuf;
    int size;
    char ch;
