...
// History
//
//	10 jan 90	peterbe		make default value of TranslateWeight()
//					FW_NORMAL instead of FW_DONTCARE.  This
//					will fix a problem with Fontware 2.0
//					download fonts.  Also, parm of this
//					fcn. changed to int, which was main
//					problem.  Changed DESC.H as well.
//
//	08 jan 90	peterbe		Fixed hp changes at end of djcharinfo()
//					for calculating width of compound
//					characters.
//
...
#define DBGweight(msg)	DBMSG(msg)
#else
#define DBGpfm(msg)	/*nulla*/
#define DBGx(msg)	/*nulla*/
#define DBGweight(msg)	/*nulla*/
#endif
...
LOCAL WORD TranslateWeight(char);
...
LOCAL BOOL djcharinfo(lpExtText, lpWidths, ch, lpCharDes, lpCHdata)
    ...
    if (lpCHdata->symbolSet != epsymDESKJET8)
	    ...
	    if (ch < 255)
			...
			{    
			// changes 25sep89, 08jan90 in following code...
			// (more efficient to use int's for array indices)
			int pos, pos1;

			pos = (int) (lpCHdata->lpTrans[ind*2]);
			pos1 = (int) (lpCHdata->lpTrans[ind*2 + 1]);

			lpWidths[ind] = ((dj[pos].width) >= (dj[pos1].width)) ?
				      dj[pos].total :	
				      dj[pos1].total;
			}
		}
...
LOCAL WORD TranslateWeight(weight)
    ...
    switch (weight)
	{
	case -8: dfWeight = FW_THIN;	    break;	// extend range
	...
	case  8: dfWeight = FW_HEAVY;	    break;	// extend range

	default: dfWeight = FW_NORMAL;	    break;	// WAS FW_DONTCARE
	}

    DBGweight(("TranslateWeight(%d) = %d\n", (int) weight, dfWeight));
