...
// History
//	15 feb 90	peterbe		corrected a comment or two.
//	02 feb 90	clarkc          lmemcpy -> lstrncpy when src is LPSTR
...
#define FIRST_KEYLEN	(4 * 2048)		/* 2048*/
...
HANDLE FAR PASCAL InitWinSF(lpAppName)
    ...
    DBGwinsf(("InitWinSF(%lp): %ls (INIT KEY NAMES)\n", lpAppName, lpAppName));
	...
	/*  Initialize the header.
         */
/* Changed lmemcpy to lstrncpy to avoid walking off end of segment
 * 
 ...
 */
	...
	#ifdef DEBUG_ListKeys
	{
	LPSTR lpS;
	int nKeys;
	int nSoft;

	for (lpS = lpKey, nSoft = 0; *lpS; )
	    {
	    if ((lpS[0] == 'S') && (lpS[1] == 'o'))
		DBGwinsf(("%ls\n", lpS));
	    while (*lpS)
...
/*  CartridgeLabel
 *
 *  Return TRUE if the passed string is a cartridge label.
 */
