...
//
// 02 feb 90    clarkc  lmemcpy() to lstrncpy() to avoid walking off segment.
 ...
 */
    ...
    #undef DBGmediumdumpfs
    #undef DBGlongdumpfs
#endif

#ifdef LOCAL_DEBUG

    // comment out the ones you don't want as: /* DBMSG() */
...
buildFontSummary(LPPCLDEVMODE lpEnviron, HANDLE hLS, LPSTR lpPortNm,
    ...
    DBGbuildfont(
	("..buildFontSummary(): Attempting to allocate hFontSummary\n"));
...
//		lmemcpy(&captionText[len], lpPortNm, sizeof(captionText)-len-1);
...
loadFontPFM(int hFile, LPFONTINFO lpFontInfo, short orient, LPFNT lpFT,
	...
	lpFT->memUsage = ((LPDRIVERINFO)lpBuf)->epMemUsage;
#else
	lpFT->memUsage = 0L;
#endif
	...
	if (!terr && lPCM && !extMOffset && !((LPDRIVERINFO)lpBuf)->xtbl.len) {
	    ...
	    DBGsoftfont(
		(".. loadFontPFM(): slimy driverinfo hack, emOrient=%d, "
	...
	lpFT->memUsage =
	    ...
	    lpFontInfo->dfAvgWidth, lpFontInfo->dfPixHeight);
#else
	lpFT->memUsage = 0L;
#endif
...
LOCAL BOOL loadSoftFont(lpFontInfo, orient, lpLS, lpFT)
    ...
    DBGentry(("loadSoftFont(%lp,%d,%lp,%lp) (NEXT SOFT FONT IN WIN.INI)\n",
    ...
    LPFNT lpFT;
{
	...
	if (lpLS->numCartridges)
		...
		*pch--=0;
		...
		/* reverse the integer string if necessary
		 * (loop above does things
		 * a little backwards...
		 */
		...
		// read the PCM filename from the font
		// installer's WIN.INI setting
...
loadResourceFont(LPFONTINFO lpFontInfo, HANDLE hModule, short orient,
	...
	if (!len) {
	    DBGerr(
	      ("..loadResourceFont(): could *not* load profile string %d\n",
		   thisFont));
	...
	if (!loadUniqueName(lpFaceName, lpFT, fnt_fontname) ||
	    ...
	    DBGerr(
    ("..loadResourceFont(): could *not* load font name or escape sequence\n"));
	    lpLS->status = FM_STRINGALLOC_FAIL;
	}
    }
...
strip_path:
	...
	if (len < FNTBL_MAXNM)
		...
		if (tmp = GlobalReAlloc(lpFT->hNMTBL,
		    ...
		    DBGloadUniqueName(
			("..loadUniqueName() %d allocated, %d free\n",
 ...
 *  it does not exist, return Roman8.
 */
...
LOCAL LPFONTSUMMARYHDR expandLockFontSum(lpHFS, lpLS, dSize)
	...
	if (!(hFontSummary = GlobalReAlloc(hFontSummary,
	    ...
	    {
	    DBGerr(
	      ("..expandLockFontSum(): could *not* realloc fontSummary\n"));
	    return (0L);
	    }
...
LOCAL void UpdateLotsOfFontsDlg(hWnd, lpFT, lpFont, count, totCount,
	...
	itoa((((lpFont->dfPixHeight - lpFont->dfInternalLeading)*72 +150)/VDPI),
	    &str[lstrlen(str)]) &&
...
LOCAL void DBGdumpFSItem(lpFontSummary, ind)
    ...
    DBMSG(
    ("   hExtMetrics=%d, hWidthTable=%d, hPairKernTable=%d, hTrackKernTable=%d\n",
	lpSummary->hExtMetrics, lpSummary->hWidthTable,
	lpSummary->hPairKernTable, lpSummary->hTrackKernTable));
    DBMSG(
    ("   ss=%s, offs=%d, indPrvSft=%d, indNxtSft=%d, LRU=%d, mUse=%ld, Z=%d, Q=%d\n",
