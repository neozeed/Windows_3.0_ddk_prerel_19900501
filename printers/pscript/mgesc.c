...
LPSTR	lpbOut;	    /* ptr to the output buffer for the esc function*/
{
	...
	case SET_BOUNDS:
		lpdv->EpsRect = *(LPRECT)lpbIn;
		lpdv->EpsRect.top    += lpdv->paper.cyMargin;
		lpdv->EpsRect.bottom += lpdv->paper.cyMargin;
		lpdv->EpsRect.right  += lpdv->paper.cxMargin;
		lpdv->EpsRect.left   += lpdv->paper.cxMargin;
		break;
