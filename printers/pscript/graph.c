...
int	cpt;
{
	...
	/* Loop once for each segment of 20 points */
...
LPRECT	lprcClip;	/* Far ptr to the clipping rectangle */
{
	...
	case OS_ELLIPSE:
		...
		PrintChannel(lpdv, "%d %d %d %d E\n",
			((lppt+1)->x - lppt->x)/2, ((lppt+1)->y - lppt->y)/2, /* dx dy */
			x, y);	/* x y */
#else
...
DO_RECT:
		...
		if (lpbr && (lpbr->lb.lbStyle == BS_PATTERN || 
			...
			PrintChannel(lpdv, "%d.5 %d.5 %d %d B\n",
...
RGB PASCAL RGBtoGray(RGB rgb)
{
	...
	ave = INTENSITY(GetRValue(rgb),GetGValue(rgb),GetBValue(rgb));

	return (RGB)RGB(ave, ave, ave);
}
...
/****************************************************************
 * RGB FAR PASCAL ColorInfo(LPDV lpdv, RGB rgb, LPCO lpco)
 *
...
RGB FAR PASCAL ColorInfo(LPDV lpdv, RGB rgb, LPCO lpco)
{
	...
	if (lpdv->fColor) {
		...
		if (GetRValue(rgb) > 127)
			color |= 0xFF;
		if (GetGValue(rgb) > 127)
			color |= 0xFF00;
		if (GetBValue(rgb) > 127)
			color |= 0xFF0000;
		return color;
	} else {
		...
		if (INTENSITY(GetRValue(rgb),GetGValue(rgb),GetBValue(rgb)) > 127)
			return RGB_WHITE;
		else
			return RGB_BLACK;
	}
...
BYTE ReOrderByte(BYTE b)
{
	static BYTE xlate[16] = {
		0x00, 0x08, 0x04, 0x0C, 0x02, 0x0A, 0x06, 0x0E,
		0x01, 0x09, 0x05, 0x0D, 0x03, 0x0B, 0x07, 0x0F};

	return (xlate[(0x0F & b)] << 4) | (xlate[(0xF0 & b) >> 4]);
}
	...
	if (fm & (FM_HATCH | FM_PATTERN)) {
		...
		if (!(lpdv->FillMode & (FM_HATCH | FM_PATTERN)) ||
			...
			PrintChannel(lpdv, "%d %d %d hC\n",
				GetRValue(HatPatColor),
				GetGValue(HatPatColor),
				GetBValue(HatPatColor));
	...
	*	the the fill pattern has changed */
	...
	!lmemIsEqual(lpdv->br.rgbPat, lpbr->rgbPat,
		...
		for (i = 0; i < sizeof(lpbr->rgbPat); i++)
			PrintChannel(lpdv, "%02x", ReOrderByte(*lpb++));
		PrintChannel(lpdv, "> p\n");
...
LPPEN	lppen;	/* Far ptr to the pen */
{
	...
	if (!lpdv->fPenSelected	|| lppen->lopn.lopnColor != lpdv->pen.lopn.lopnColor) {
		PrintChannel(lpdv, "%d %d %d pC\n",
			GetRValue(lppen->lopn.lopnColor),
			GetGValue(lppen->lopn.lopnColor),
			GetBValue(lppen->lopn.lopnColor));
