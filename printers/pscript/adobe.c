	...
	int bufSize;
{
	...
	do{
		if (_lread(fh, (char *)&hdr, sizeof(HDR))!=sizeof(HDR))
		    goto ERR;
	...
	BOOL fAscii;	/* TRUE-->convert to 2 ASCII chars */
{
	...
	for(i=0;i<nBuffers;i++){
		...
		if(_lread(fh, buf, bufSize)!=bufSize) goto ERR;
#ifdef DEBUG_ON
...
/*			PrintChannel(lpdv,(LPSTR)"\n"); */
	...
	if(nRemaining>0){
		...
		if(_lread(fh, buf, nRemaining)!=nRemaining) goto ERR;
#ifdef DEBUG_ON
DBMSG(((LPSTR)" PSWriteSpool():"));
	...
	int bufSize;
{
	...
	for(i=bufSize-1;i>=0;i--){
		...
		buf[--j]=(char)Nibble2HexChar(temp & 0x0f);
		buf[--j]=(char)Nibble2HexChar(temp>>4);
