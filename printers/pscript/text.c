...
long	PASCAL DumpStr(LPDV, LPFX, LPDRAWMODE, int, int, BOOL, LPSTR, long, int, int, int);
...
long PASCAL DumpStr(lpdv, lpfx, lpdm, ix, iy, fPrint, lpbStr, cxStr, count, cBrk, cxBrk)
...
int	cxBrk;	/* The pixel width added to break characters */
{
	...
	if (fPrint && cxStr) {
		...
		/* SB == StrBlt */
		PrintChannel(lpdv, "%d %d %d %q SB\n",
		    ix, iy, (int)cxStr, lpbStr, count);
	}
...
LPSHORT lpdx;	/* Far ptr to delta-x moves */
{
	...
	if (fPrint && (lpdm->bkMode == OPAQUE)) {
		...
		if (LOWORD(opaq) && HIWORD(opaq)) {
			...
			OpaqueBox(lpdv, ix, iy, LOWORD(opaq), HIWORD(opaq), lpdm->bkColor);
	...
	for (cxSrc = cxStr = 0L, lpbStr = lpbSrc,
		...
		if (runbreak) {
			...
			ix += Scale(cxStr + runbreak, RCos(1000, lpfx->escapement), 1000);
			iy -= Scale(cxStr + runbreak, RSin(1000, lpfx->escapement), 1000);
...
void FAR PASCAL OpaqueBox(LPDV lpdv, int x, int y, int dx, int dy, RGB color)
{
	PrintChannel(lpdv, "%d %d %d %d %d %d %d OB\n",
		dx, dy, x, y,
		GetRValue(color),
		GetGValue(color),
		GetBValue(color));
...
WORD	options;	/* 2.0+ option switches */
{
	...
	if (cbSrc > 0 && lprcOpaq) {
		...
		OpaqueBox(lpdv, lprcOpaq->left, lprcOpaq->top,
...
LPFX lpfx;	/* Far ptr to extra font info for driver */
{
	...
	if (lpfx->lid != lpdv->lidFont || lpfx->lid == -1L) {
		...
		PrintChannel(lpdv, "%d %d %d %d %d %d %d %d ",
