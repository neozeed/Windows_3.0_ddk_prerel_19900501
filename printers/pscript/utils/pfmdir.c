 ...
 *
 * purpose:
 * 	create .dir resource files from .lst files for fonts
 *	(are other inputs used?)
 *
 *
 * usage:
 *	pfmdir: <file.lst>
	...
	char **argv;
{
    ...
    if (argc<2)
	{
		printf("pfmdir: expected a .lst filename as an argument\n");
		exit(1);
	}
    else if (argc>2)
	{
		printf("pfmdir: too many arguments\n");
		exit(1);
	}
    LoadNames(*++argv);
    MakeDir();
