...
LPTEXTXFORM lpXform    ;
{
	...
	dwRet = chStrBlt(lpDevice, x, y, lpString, count,lpFont, lpDrawMode,lpXform, (LPRECT) lpClipRect);
      }
    else if (lpDevice->epType)
      {
	lpDevice->epBmpHdr.bmBits = lpDevice->epBmp;
	...
	goto dm;
      ...
      {
dm:
	dwRet = dmStrBlt(lpDevice, x, y, lpClipRect,
		 lpString, count, lpFont, lpDrawMode, lpXform);
...
#if NO_WEP
#else
void FAR PASCAL WEP(int fExitWindows)
{
}
#endif
