...
no_print:
	 ...
	 */
		...
		for (size = strlen = wordlen = 0; strlen < count; strlen++) {
			...
			wordsize = LOWORD(str_out(lpDevice,
        ...
        if (fFree)      /* translation took place */
			    ...
			    (c = Translate(lpString[i], 1)) <= SPACE)
                        ...
                        lpDrawMode->BreakErr = newDDA;
			skip = LOWORD(str_out(lpDevice, lpString2, - i, lpFont, lpDrawMode, lpXform));
#if COLOR
			/* do str_out and InsertString for each color */
...
short x, y, size;
{
	...
	if (InsertPQ(lpDevice->epYPQ, tag, y) == PQERROR)
		if (SizePQ(lpDevice->epYPQ, Y_INIT_ENT) == PQERROR)
                        {
