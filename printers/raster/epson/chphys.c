...
LPTEXTXFORM lpXform;
{
        ...
        if (real = (count > 0)) {
#ifdef TOSHIBA
	    myWrite(lpDevice, (LPSTR) lpFont + ((LPPRDFONTINFO)lpFont)->bfont.offset, ((LPPRDFONTINFO)lpFont)->bfont.length);
#else
	     ...
	     */
			if ((lpFont->dfPitchAndFamily & 0x1) && width)
			    ps_char_out(lpDevice, *lpString, width, lpFont);
			else
#endif
...
#elif !defined(NOFILTER)
			/* be careful not to print 128-159... printer
			 * will vomit and have a seizure
			 */
			for (;count;count--,lpString++)
			  {
			    char ch='.';

			    if ((unsigned char)*lpString < 127
				  || (unsigned char)*lpString >= 160)
				myWrite(lpDevice,lpString,1);
			    else
				myWrite(lpDevice,&ch,1);
			  }
#else
...
#ifdef TOSHIBA
		myWrite(lpDevice, (LPSTR) lpFont + ((LPPRDFONTINFO)lpFont)->efont.offset, ((LPPRDFONTINFO)lpFont)->efont.length);
#else
                myWrite(lpDevice, (LPSTR) pcsb + ((LPPRDFONTINFO)lpFont)->efont.offset, ((LPPRDFONTINFO)lpFont)->efont.length);
#endif
                return size;
        } else
		return MAKELONG(size, lpFont->dfPixHeight);
}
#endif
...
short width;
{
	...
	/* most printers don't like these
	 */
	if (c>=127 && c<160)
	    c = '.';
...
LPRECT  lpRect ;
{
        ...
        if (fFree)
		GlobalFree(HIWORD((long)lpString2));
