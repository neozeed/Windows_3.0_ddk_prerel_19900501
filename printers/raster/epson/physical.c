...
short y;
{
	 ...
	 */
	z = lpDevice->epPageWidth / 2;


	for (tmp = 0; tmp < lpDevice->epPageWidth / 2; tmp++)
	  {
	    bwbuf[tmp] = ~(bufptr[2*z] |
			   bufptr[ z ] |
			   bufptr[ 0 ]);
	    bufptr++;
	  }
...
#ifdef IBMCOLOR
	if (lpDevice->epRibbon)
	  {
	    /* Using an RGBK ribbon, with the black areas handled above.
	     * Since RGB inks don't mix well (that's what CMY is for)
	     * we'll only print pixels with a single color bit set, leaving
	     * others white.  Also, switch red and blue, since the colors
	     * are in an inconvenient order on the ribbon.
	     */
	    bufptr = (WORD far *)buf;
	    for (tmp = 0; tmp < lpDevice->epPageWidth / 2; tmp++)
	      {
		/* set bwbuf to the bits that can be printed
		 */
		bwbuf[tmp] = ~((bufptr[0] & bufptr[ z ])
			     | (bufptr[z] & bufptr[2*z])
			     | (bufptr[0] & bufptr[2*z]));
	    ...
	    /* now clear the bits to be left white
	     */
	    bufptr = (WORD far *)buf;
	    for (z = NPLANES; z; z--)
	      {
		for (tmp=0; tmp<lpDevice->epPageWidth/2; tmp++, bufptr++)
		  {
	...
	/* now print out each of the three colors
	 * note that we're assume C and R, M and G, Y and B in corresponding
	 * positions.
	 */
...
LPDEVICE lpDevice;
{
    ...
    if (MinPQ(lpDevice->epYPQ) != PQERROR)
