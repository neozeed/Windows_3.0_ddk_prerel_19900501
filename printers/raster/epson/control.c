		...
		if (lpDevice->epDoc != TRUE)
		  {
			...
			return TRUE;
		      ...
		      {
			lpDevice->epDoc = FALSE;
			return lpDevice->epJob;     /* error code */
		      }
		  }
		return -1;			    /* job already started */
...
/* close the job */		
        ...
        case GETPHYSPAGESIZE:
		lpOutData->x = lpDevice->epPF->XPhys;
		lpOutData->y = lpDevice->epPF->YPhys;
                break;

        case GETPRINTINGOFFSET:
		lpOutData->x = lpDevice->epPF->XPrintingOffset;
		lpOutData->y = lpDevice->epPF->YPrintingOffset;
                break;
