...
stack_top	equ	0Ah		;Stack probe location
...
;	*********************************************************************
;       AS THIS FILE IS BASED ON BUILDSTR.ASM IN THE DISP30 TREE OF 4 PLANE 
...
p_real_build_string proc near
	...
	mov	cx,CodeOFFSET p_non_justified_text
	test	bl,WEIRD_SPACING
	jz	short p_build_have_proc
	mov	cx,CodeOFFSET p_justified_text
...
p_build_worst_ok:
	mov	cx,CodeOFFSET p_worst_case
	mov	bl,accel
	mov	ah,excel
...
p_pad_right_hand_side	proc near
	...
	jz	short p_prhs_new_rhs	;  move text_bbox to byte boundary
	mov	bl,excel
	not	bl
	test	bl,OPAQUE_RECT+BOUNDED_IN_Y
