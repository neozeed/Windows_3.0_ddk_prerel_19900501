#define NO_EPSSTRIP	1
#define NO_XMOVETO	1
#define NO_YMOVETO	1
#define NO_CH_LINE_OUT	1
#define NO_DUMP 	1

#include "..\..\epson\physical.c"
...
LPDEVICE lpDevice;
{
    ...
    if (lpDevice->epType == DEV_LAND)
        {
        offset = lpDevice->epXOffset - BAND_WIDTH;

        for (x = 0; x < BAND_LINE;x++)
            {
            temp = buffer + PG_ACROSS * lpDevice->epBmpHdr.bmPlanes;
	    BitOffset = NPINS * x;
	    BitRem = BitOffset % 8;
            bmp = (LPSTR) lpDevice->epBmp + BitOffset / 8;
	    for (bmp0 = bmp, z = lpDevice->epBmpHdr.bmPlanes; z; z--) {
		bmp = bmp0 + (z - 1) * (BAND_SIZE(0) / NPLANES);
                for (y = PG_ACROSS; y; --y, bmp += BAND_WIDTH / 8) {
		    tmp1 = (unsigned char) *bmp << (unsigned char) BitRem;
		    tmp2 = (unsigned char) *(bmp + 1) >> (unsigned char) (8- BitRem);
                    *--temp = (tmp1 + tmp2) | 1;
                }
	    }
            color_line_out(lpDevice, buffer, PG_ACROSS, x * NPINS + offset);
            }
        }
    else    /* portrait */
    {
        bmp = lpDevice->epBmp;
        offset = lpDevice->epYOffset - BAND_WIDTH;
        for (x = 0; x < BAND_LINE; x++)
	{
	    for (z = lpDevice->epBmpHdr.bmPlanes; z; z--) {
		dmTranspose(bmp + (z - 1) * (BAND_SIZE(0) / NPLANES),
			    buffer + (z - 1) * PG_ACROSS,
			    PG_ACROSS >> 3);
	    }
	    ...
	    /* set all low bits so that line_out can output 8-bit bytes 
	       instead of 7-bit bytes */
            for (y = 0; y < PG_ACROSS * lpDevice->epBmpHdr.bmPlanes; y++) {
            	*temp |= 1;
            	temp++;
            }
            color_line_out(lpDevice, buffer, PG_ACROSS, x * NPINS + offset);
            bmp += PG_ACROSS / 8 * NPINS;
        }
    }
